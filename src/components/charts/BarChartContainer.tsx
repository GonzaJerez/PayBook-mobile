...
export const BarChartContainer = ({children, title, maxToGraph, data, color}: Props) => {
  ...
  return (
    <PrincipalCardContainer size='big'>
      ...
      <View style={styles.chartContainer}>
        <View style={[styles.yAxisLine, {borderColor: theme.colors.border}]} />
        <View style={[styles.xAxisContainer, {borderColor: theme.colors.border}]}>
          ...
          <View style={styles.barsTextContainer}>
            {data.map((data, idx) => (
              <View key={idx}>
                <Text style={{color: color[idx] || color[0]}}>{data}</Text>
              </View>
            ))}
          </View>
