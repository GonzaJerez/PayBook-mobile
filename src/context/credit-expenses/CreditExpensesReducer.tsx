...
type CreditExpenseActions =
  ...
  | {type: 'setActualCreditExpense', payload: {creditExpense: CreditPayment}}
  | {type: 'updateCreditExpense', payload: {creditExpense: CreditPayment}}
  | {type: 'removeCreditExpense', payload: {idCreditExpense: string}}

export const CreditExpensesReducer = (state: CreditExpenseState, action: CreditExpenseActions): CreditExpenseState => {
  ...
  switch (action.type) {
    ...
    case 'updateCreditExpense':
      return {
        ...state,
        actualCreditExpense: action.payload.creditExpense,
        allCreditExpenses: state.allCreditExpenses
          .map(credExp => credExp.id === action.payload.creditExpense.id
    ...
    case 'removeCreditExpense':
      return {
        ...state,
        allCreditExpenses: state.allCreditExpenses
          .filter(credExp => credExp.id !== action.payload.idCreditExpense),
